...
setwd(here("data", "sequences", "clean", "unmapped"))
...
write(asv_fasta, "dada2_c_um_asvs.fasta")
...
setwd(here("data", "sequences", "clean", "predator"))
samples <- scan("samples", what = "character")
...
#code, just make sure you're in that working directory
#ls *_R1.fq | cut -f1-4 -d "_" > samples
setwd(here("data", "sequences", "clean", "prey"))
samples <- scan("samples", what = "character")
...
setwd(here("data", "ASV_lists"))
asv_fasta <- c(rbind(asv_headers, asv_seqs))
write(asv_fasta, "dada2_c_prey_asvs.fasta")

# count table:
setwd(here("data", "ASV_tables"))
asv_tab <- t(seqtab.nochim)
row.names(asv_tab) <- sub(">", "", asv_headers)
write.table(asv_tab, "dada2_c_prey_asv_tab.tsv", sep="\t", quote=F, col.names=NA)
# tax table:
#asv_tax <- taxa
#row.names(asv_tax) <- sub(">", "", asv_headers)
#write.table(asv_tax, "ASVs_taxonomy.tsv", sep="\t", quote=F, col.names=NA)
